...
public fun list_hero(nft: Hero, price: u64, ctx: &mut TxContext) {
    let list_hero = ListHero { id: object::new(ctx), nft, price, seller: ctx.sender() };
...
public fun buy_hero(list_hero: ListHero, coin: Coin<SUI>, ctx: &mut TxContext) {
    ...
    event::emit(HeroBought { list_hero_id: object::uid_to_inner(&id), price, buyer: ctx.sender(), seller, timestamp: ctx.epoch_timestamp_ms() });
    object::delete(id);
}
